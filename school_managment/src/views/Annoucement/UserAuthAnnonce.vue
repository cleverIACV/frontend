<template>
  <div class="min-h-screen bg-gray-100">
    ...
    <div class="relative">
      ...
      >
        <h1 class="text-4xl font-bold">Mes annonces</h1>
      </div>
    </div>
    ...
    <main class="container mx-auto px-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-3">
          ...
          <div v-if="userAnnonces.length > 0" class="space-y-4">
            <div
            ...
            >
              <div class="flex justify-between items-start">
                <div 
                    ...
                    @click="viewAnnonceDetail(annonce.id)">
                ...
                <div class="flex space-x-2">
                  <button
                    @click="updateAnnonce(annonce.id)"
                    class="bg-gray-200 text-black py-2 px-4 rounded-lg"
                  >
                    <i class="fas fa-edit"></i>
                  </button>
                  <button
                    @click="confirmDeleteAnnonce(annonce.id)"
          ...
          <div v-else class="text-center">Aucune annonce trouvée</div>
        </div>
      </div>
    </main>
  </div>
...
export default {
  ...
  computed: {
    ...
    userAnnonces() {
      return this.getUserAuthAnnounceData || [];
    },
    loading() {
  ...
  methods: {
    ...mapActions("annoucement", ["getUserAuthAnnonce", "deleteAnnonce"]),
    viewAnnonceDetail(annonceId) {
      this.$router.push(`/annonce-details/${annonceId}`);
    },
    createAnnonce() {
      this.$router.push(`/create-announcement/step-1`);
    },
    updateAnnonce(annonceId) {
      this.$router.push(`/update-annonce/${annonceId}`);
    },
    confirmDeleteAnnonce(annonceId) {
      if (confirm("Êtes-vous sûr de vouloir supprimer cette annonce ?")) {
        this.deleteAnnonce(annonceId).then(() => {
          this.getUserAuthAnnonce(); // Refresh the list after deletion
        });
      }
    },
  },
  mounted() {
    this.getUserAuthAnnonce().then(() => {
      console.log("Annonces:", this.userAnnonces);
    });
  },
};
</script>
